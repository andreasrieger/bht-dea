...
function runOutput(valid, output) {
  ...
  autoRun.addEventListener('click', () => { runAuto(output); });
  oneSecondDelay.addEventListener('click', () => { runDelayed(output, 1); });
  twoSecondDelay.addEventListener('click', () => { runDelayed(output, 2); });
  fourSecondDelay.addEventListener('click', () => { runDelayed(output, 4); });
}
...
*/
function getRandomSequence(alphabet) {// To do: set min and max
  const
    n = alphabet.length,
    arr = [],
    x = randomInt(5, 10)
